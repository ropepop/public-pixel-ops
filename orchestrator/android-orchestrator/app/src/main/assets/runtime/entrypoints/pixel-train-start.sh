...
CONF_ENV="/data/local/pixel-stack/conf/apps/train-bot.env"
RUNTIME_ENV="${BASE}/env/train-bot.env"
TPL_DIR="/data/local/pixel-stack/templates/train"
BIN_DIR="${BASE}/bin"
RUN_DIR="${BASE}/run"
LOG_DIR="${BASE}/logs"
PID_FILE="${RUN_DIR}/train-bot-service-loop.pid"
LOOP_BIN="${BIN_DIR}/train-bot-service-loop"
TUNNEL_LOOP_PID_FILE="${RUN_DIR}/train-web-tunnel-service-loop.pid"
TUNNEL_LOOP_BIN="${BIN_DIR}/train-web-tunnel-service-loop"
LAUNCH_BIN="${BIN_DIR}/train-bot-launch"
TPL_LAUNCH="${TPL_DIR}/train-launch.sh"
TPL_LOOP="${TPL_DIR}/train-service-loop.sh"
TPL_TUNNEL_LOOP="${TPL_DIR}/train-web-tunnel-service-loop.sh"

mkdir -p "${BIN_DIR}" "${RUN_DIR}" "${LOG_DIR}" "${BASE}/env" "${BASE}/data/schedules" "${BASE}/state"
...
ensure_web_session_secret() {
  if ! is_true "${TRAIN_WEB_ENABLED:-0}"; then
    return 0
  fi

  secret_file="${TRAIN_WEB_SESSION_SECRET_FILE:-/data/local/pixel-stack/conf/apps/train-bot-web-session-secret}"
...
start_loop_if_needed() {
  ...
  nohup env TRAIN_BOT_ROOT="${BASE}" "${loop_bin}" >>"${log_file}" 2>&1 &
  pid="$!"
  sleep 1
...
ensure_train_web_tunnel_loop_state() {
  ...
  if [ ! -x "${TUNNEL_LOOP_BIN}" ]; then
    echo "missing train web tunnel loop: ${TUNNEL_LOOP_BIN}" >&2
    return 1
  fi
  start_loop_if_needed "${TUNNEL_LOOP_PID_FILE}" "${TUNNEL_LOOP_BIN}" "${LOG_DIR}/train-web-tunnel-service-loop.log"
}

if [ ! -f "${TPL_LAUNCH}" ]; then
  echo "missing train launch template: ${TPL_LAUNCH}" >&2
  exit 1
fi
if [ ! -f "${TPL_LOOP}" ]; then
  echo "missing train loop template: ${TPL_LOOP}" >&2
  exit 1
fi
if [ ! -f "${TPL_TUNNEL_LOOP}" ]; then
  echo "missing train tunnel loop template: ${TPL_TUNNEL_LOOP}" >&2
  exit 1
fi
...
if [ ! -x "${BASE}/bin/train-bot.current" ] && [ ! -x "${BASE}/bin/train-bot" ]; then
  echo "missing train bot executable under ${BASE}/bin" >&2
  exit 1
fi
...
nohup env TRAIN_BOT_ROOT="${BASE}" "${LOOP_BIN}" >>"${LOG_DIR}/service-loop.log" 2>&1 &
pid="$!"
sleep 1
if pid_matches_target "${pid}" "${LOOP_BIN}"; then
  echo "${pid}" > "${PID_FILE}"
  ensure_train_web_tunnel_loop_state || exit 1
