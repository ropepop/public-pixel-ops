...
while true; do
  stale_bot_pid="$(cat "${BOT_PID_FILE}" 2>/dev/null || true)"
  if [ -n "${stale_bot_pid}" ] && ! kill -0 "${stale_bot_pid}" >/dev/null 2>&1; then
    rm -f "${BOT_PID_FILE}" >/dev/null 2>&1 || true
  fi

  log "starting satiksme bot"
  "${LAUNCH_BIN}" >> "${LOG_DIR}/satiksme-bot.log" 2>&1 &
  child_pid="$!"
  ...
  child_rc=$?
  set -e
  log "satiksme bot exited rc=${child_rc}"
